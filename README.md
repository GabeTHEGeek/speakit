...
- Invisible audio-host wake-up when macOS suspends microphone reacquisition after the main window is closed, with focus returned to the original app
...
If a call app such as Google Meet is opening, closing, or switching microphones, SpeakIt cancels a stalled recording start instead of remaining frozen. It also detects a stale or muted warmed-up microphone track and requests a fresh one on the next attempt. If macOS suspends that request while SpeakIt's main window is closed, SpeakIt briefly wakes its invisible audio host, reacquires the microphone, and returns focus to the original app. A connection delivered after the first wait times out is retained, and retries reuse the same pending request rather than starting competing microphone requests.
